...
test.describe('Live Service Integration Tests', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    try {
      const apiHealth = await request.get('http://localhost:7778/health');
  ...
  test('Live API endpoints return real data', async ({ request }) => {
    ...
    for (const endpoint of endpoints) {
      const response = await request.get(`http://localhost:7778${endpoint}`);
  ...
  test('Frontend loads and connects to live API', async ({ page }) => {
    ...
    page.on('request', request => {
      if (request.url().includes('localhost:7778')) {
  ...
  test('Real-time agent message communication', async ({ page, request }) => {
    ...
    const response = await request.post('http://localhost:7778/api/dashboard/agents/demo-developer-1/message', {
      data: {
        message: message,
        context: { 
          test: 'live-ui-test',
    ...
    const historyResponse = await request.get('http://localhost:7778/api/dashboard/agents/demo-developer-1/conversation');
  ...
  test('Task submission to live agents', async ({ request }) => {
    ...
    const response = await request.post('http://localhost:7778/api/dashboard/agents/demo-architect-1/task', {
      data: taskData,
      headers: {
        'Content-Type': 'application/json'
      }
  ...
  test('Live statistics update correctly', async ({ request }) => {
    // Get initial stats
    const stats1 = await request.get('http://localhost:7778/api/dashboard/stats');
    const data1 = await stats1.json();
    
    console.log('Initial statistics:', data1);
    
    // Submit a task to change stats
    await request.post('http://localhost:7778/api/dashboard/agents/demo-qa-1/task', {
      data: {
    ...
    // Get updated stats
    const stats2 = await request.get('http://localhost:7778/api/dashboard/stats');
  ...
  test('Multiple agents handle concurrent messages', async ({ request }) => {
    ...
    const promises = agents.map(agentId => 
      request.post(`http://localhost:7778/api/dashboard/agents/${agentId}/message`, {
        data: {
  ...
  test('Error handling for invalid agent IDs', async ({ request }) => {
    const response = await request.post('http://localhost:7778/api/dashboard/agents/invalid-agent-id/message', {
      data: {
  ...
  test('Verify all services are truly independent', async ({ request }) => {
    ...
    const directAPICall = await request.get('http://localhost:7778/api/dashboard/agents');
    ...
    const secondCall = await request.get('http://localhost:7778/api/dashboard/agents');
